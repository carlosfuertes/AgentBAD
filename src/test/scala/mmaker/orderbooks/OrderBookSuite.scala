...
class OrderBookSuite extends FunSuite {
  ...
  test("OrderBooks should be able to process limit orders correctly, fullfilling previous order and enqueing partial orders") {
    ...
    assert(bookOwner.messages.length === 5)
    ...
    assert(bookOwner.messages(2) === (AggregatedBookOwner.ORDER_PROGRESS, ask1, 1000, Currency(10)))
    assert(bid1.status === Order.PARTIALLY_FILLED)
    assert(bookOwner.messages(3) === (AggregatedBookOwner.ORDER_COMPLETED, ask1))
    assert(ask1.openAmount === 0)
    assert(ask1.status === Order.FILLED)
    assert(bookOwner.messages(4) === (AggregatedBookOwner.TRADE_NOTIFICATION, Order.BUY, 1000, Currency(10)))
    bookOwner.emptyMessages()
    ...
    assert(bookOwner.messages.length === 11)
    ...
    assert(bookOwner.messages(2) === (AggregatedBookOwner.ORDER_PROGRESS, bid1, 500, Currency(15)))
    assert(bookOwner.messages(3) === (AggregatedBookOwner.ORDER_COMPLETED, bid1))
    assert(bookOwner.messages(4) === (AggregatedBookOwner.TRADE_NOTIFICATION, Order.SELL, 500, Currency(15)))

    assert(bookOwner.messages(5) === (AggregatedBookOwner.QUOTE_NOTIFICATION, Order.ASK, 2500, Currency(11)))
    assert(bookOwner.messages(6) === (AggregatedBookOwner.ORDER_PROGRESS, sell1, 500, Currency(5)))
    assert(bookOwner.messages(7) === (AggregatedBookOwner.ORDER_PROGRESS, bid2, 500, Currency(5)))
    assert(bookOwner.messages(8) === (AggregatedBookOwner.ORDER_COMPLETED, bid2))
    assert(bookOwner.messages(9) === (AggregatedBookOwner.TRADE_NOTIFICATION, Order.SELL, 500, Currency(5)))
    assert(bookOwner.messages(10) === (AggregatedBookOwner.ORDER_COMPLETED, sell1))
